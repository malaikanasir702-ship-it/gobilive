...
export function registerStreamSignaling(io: Server) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('toggle_call', (data: { roomId: string; enabled: boolean }) => {
      if (!data?.roomId) return;
    ...
    socket.on('accept_call', (data: { roomId: string; username: string }) => {
      if (!data?.roomId) return;
    ...
    socket.on('end_call', (data: { roomId: string; username: string }) => {
      if (!data?.roomId) return;
    ...
    socket.on('toggle_moderator', (data: { roomId: string; userId: string; username: string; assign: boolean }) => {
      if (!data?.roomId) return;
