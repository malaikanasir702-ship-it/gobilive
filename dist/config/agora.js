...
exports.buildAgoraRtcToken = buildAgoraRtcToken;
exports.buildSeatToken = buildSeatToken;
exports.seatIndexToAgoraUid = seatIndexToAgoraUid;
exports.userIdToAgoraUid = userIdToAgoraUid;
const agora_access_token_1 = require("agora-access-token");
const live_model_1 = __importDefault(require("../features/live/live.model"));
// ─────────────────────────────────────────────
// Original single-role token builder (UNCHANGED)
// ─────────────────────────────────────────────
function buildAgoraRtcToken(channelName, uid = 0, role = 'publisher', expireSeconds = 3600) {
    ...
    return agora_access_token_1.RtcTokenBuilder.buildTokenWithUid(appId, certificate, channelName, uid, agoraRole, privilegeExpire);
}
/**
 ...
 */
async function buildSeatToken(channelName, userId, expireSeconds = 3600) {
    const appId = process.env.AGORA_APP_ID || '';
    const certificate = process.env.AGORA_PRIMARY_CERTIFICATE || '';
    ...
    if (seat) {
        const token = buildAgoraRtcToken(channelName, seat.agoraUid, 'publisher', expireSeconds);
        return {
            uid: seat.agoraUid,
            token,
            role: 'publisher',
 ...
 */
function seatIndexToAgoraUid(seatIndex) {
