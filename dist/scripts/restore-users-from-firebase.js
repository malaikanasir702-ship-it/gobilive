...
async function main() {
    ...
    const listUsersResult = await firebase_admin_1.default.auth().listUsers(1000);
    ...
    for (const fUser of firebaseUsers) {
        ...
        const queryConditions = [{ googleId: uid }];
        if (email) {
        ...
        const passwordHash = await bcryptjs_1.default.hash(randomPass, 10);
