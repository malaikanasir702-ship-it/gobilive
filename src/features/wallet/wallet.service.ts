...
export async function requestDiamondWithdrawal(
  ...
  const { CountryPolicy } = await import('../policy/country-policy.model');
  const userCountry = (user as any).country ? (user as any).country.toUpperCase() : 'PK';
  ...
  if ((user as any).agencyId) {
    resolvedAgencyId = (user as any).agencyId;
  }
  ...
  if (payoutDetails) {
    const parts = payoutDetails.split('|').map((s: string) => s.trim());
