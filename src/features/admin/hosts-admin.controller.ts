...
export const listHosts = async (req: AdminAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const filter: any = {};
    // Hosts are users assigned to an agency (convention in this codebase)
    filter.agencyId = { $exists: true, $ne: null };

    if (search) {
      const re = new RegExp(search, 'i');
      filter.$or = [{ username: re }, { _id: search }];
    }
...
// Get host profile with withdrawals, beans, diamonds, social stats
...
export const blockHost = async (req: AdminAuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { type, duration } = req.body; // similar semantics as user block
    ...
    await logActivity({
      ...
      actionType: 'block_host',
      targetEntityType: 'User',
      targetEntityId: id,
      description: `Blocked host ${user.username} (${update.blockType})`,
    });
...
export const unblockHost = async (req: AdminAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await logActivity({
      actorId: req.adminUser!.id,
      actorRole: req.adminUser!.role,
      actionType: 'unblock_host',
      targetEntityType: 'User',
      targetEntityId: id,
      description: `Unblocked host ${user.username}`,
    });
    res.status(200).json({ success: true, user });
  } catch (err: any) {
    res.status(500).json({ success: false, message: err.message });
...
export const approveHost = async (req: AdminAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await logActivity({
      ...
      targetEntityType: 'User',
      targetEntityId: id,
      description: `Approved host ${user.username}`,
    });
    res.status(200).json({ success: true, user });
  } catch (err: any) {
    res.status(500).json({ success: false, message: err.message });
...
export const disapproveHost = async (req: AdminAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await logActivity({
      actorId: req.adminUser!.id,
      actorRole: req.adminUser!.role,
      actionType: 'disapprove_host',
      targetEntityType: 'User',
      targetEntityId: id,
      description: `Disapproved host ${user.username}. Reason: ${reason || 'N/A'}`,
    });
    res.status(200).json({ success: true, user });
  } catch (err: any) {
    res.status(500).json({ success: false, message: err.message });
...
export const transferHostAgency = async (req: AdminAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await logActivity({
      actorId: req.adminUser!.id,
      actorRole: req.adminUser!.role,
      actionType: 'transfer_host_agency',
      targetEntityType: 'User',
      targetEntityId: id,
      description: `Transferred host ${user.username} to agency ${targetAgencyId}`,
    });
    res.status(200).json({ success: true, user });
  } catch (err: any) {
    res.status(500).json({ success: false, message: err.message });
