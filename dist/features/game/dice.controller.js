...
exports.getDiceConfig = exports.rollDice = void 0;
...
function rollDie() {
    return crypto_1.default.randomInt(1, 7); // 1-6 inclusive
}
const rollDice = async (req, res) => {
    try {
        ...
        if (!['over', 'under', 'exact'].includes(betType)) {
            res.status(400).json({ success: false, message: 'Invalid bet type.' });
            return;
        }
        if (betType === 'exact' && (!exactSum || exactSum < 2 || exactSum > 12)) {
            res.status(400).json({ success: false, message: 'exactSum must be 2–12.' });
            return;
        }
...
exports.getDiceConfig = getDiceConfig;
