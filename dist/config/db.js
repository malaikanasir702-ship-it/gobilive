...
async function syncMultiRoleIndexes() {
    try {
        ...
        // NOTE: Legacy rcoins→beanWallet migration has been permanently disabled.
        // It was running on EVERY restart and doubling users' beans (beanWallet + rcoins added each time).
        // beanWallet is the single source of truth. rcoins field is fully deprecated.
    }
    catch (err) {
        console.warn('⚠️ Index / account sync warning:', err.message);
    }
}
// syncLegacyRcoinsToBeans — PERMANENTLY REMOVED
// This function was adding rcoins on top of beanWallet on EVERY server restart,
// causing users' bean balances to grow automatically after each deploy.
// beanWallet is the sole source of truth going forward.
