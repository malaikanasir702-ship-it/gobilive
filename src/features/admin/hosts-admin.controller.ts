  ...
  try {
    ...
    try {
      ...
      for (const req of approvedRequests) {
        ...
        const matchOr: any[] = [];
        if (fd.parentId && Types.ObjectId.isValid(String(fd.parentId))) {
          matchOr.push({ _id: fd.parentId });
        }
        if (fd.email) matchOr.push({ email: fd.email.toLowerCase().trim() });
        if (fd.phone) matchOr.push({ phone: fd.phone.trim() });
        if (matchOr.length === 0) continue;

        const updateDoc: any = { role: 'host', $addToSet: { badges: 'host' } };
        if (resolvedAgencyId) updateDoc.agencyId = resolvedAgencyId;

        const noAgencyFilter: any = { $and: [{ $or: matchOr }, { $or: [{ agencyId: { $exists: false } }, { agencyId: null }, { agencyId: '' }] }] };
        await User.updateMany(noAgencyFilter, updateDoc);
        // Also ensure role/badge on users that already have agencyId set
        await (User.updateMany as any)({ $or: matchOr }, { role: 'host', $addToSet: { badges: 'host' } });
      }
    } catch (_) {}
    ...
    if (role === 'agency' || role === 'sub_agency') {
      ...
      const myAgencies = await Agency.find({ superAdminId: req.adminUser!.id } as any).select('_id agencyCode').lean();
      if (!myAgencies.length) {
        res.status(200).json({ success: true, hosts: [], total: 0, page, totalPages: 0 });
        return;
      }
      const agencyRefs: string[] = [];
      for (const a of myAgencies) { agencyRefs.push(String(a._id)); if (a.agencyCode) agencyRefs.push(a.agencyCode); }
      filter.agencyId = { $in: agencyRefs };
    } else if (role === 'sub_admin') {
      // sub_admin sees only hosts belonging to their agencies
      const myAgencies = await Agency.find({ subAdminId: req.adminUser!.id } as any).select('_id agencyCode').lean();
      if (!myAgencies.length) {
        res.status(200).json({ success: true, hosts: [], total: 0, page, totalPages: 0 });
        return;
      }
